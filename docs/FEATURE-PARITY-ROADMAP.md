...
> **포스트잇(P3-A)·알림(P3-B) 은 비채택으로 결정됨 (사용자 2026-06).** §4 참조.
...
- **포스트잇 (post_it)** — 섹션별 메모. *사용자 결정(2026-06) 으로 비채택* —
  메모는 거래 annotation/해시태그(시나리오 06) 로 충분, 별도 메모 패널 불요.
- **알림 (notifications)** — *사용자 결정(2026-06) 으로 비채택* — TUI 는
  세션형 도구라 ≥5분 백그라운드 폴링/배지의 가치가 낮음.
...
5. **P2-C / P3-C** report_customs 쓰기 · 섹션 CRUD · 계정 정렬.
