...
│       ├── sync.py              # 동기화 facade — 코어는 이것만 호출(백엔드 선택 none|p4)
│       ├── p4_sync.py           # P4 백엔드 구현 (sync.py 뒤에 숨음) — 자동 submit + 동기화 검사
...
3. **로컬 sqlite mirror + 선택적 동기화 백엔드** — 후잉 API 가 모르는 메모/
   태그/첨부는 sqlite (`<project>/db/whooing-data.sqlite`) 에 별도 보관. 머신
   간 동기화는 **`sync.py` facade** 경유 — 코어(app/data/repository/screens/
   cli)는 `sync.submit_db`/`submit_files`/`flush_on_exit` 등만 호출하고
   `p4_sync` 를 직접 import 하지 않는다. 백엔드는 `[sync] backend`(또는 env
   `WHOOING_SYNC_BACKEND`)로 선택: **기본 `none`(동기화 안 함 — P4 완전 무시),
   `p4`(Perforce)**. `none` 이면 모든 동작 no-op·질의 False/0 → 비-P4 사용자는
   시작 검사/종료 flush/알림 전부 발생 안 함. 새 백엔드는 facade 함수에 분기만
   추가(코어 호출부 불변).
...
| 동기화 백엔드 (켜고 끄기) | `sync.py`(facade), `config.py` `[sync] backend`, `docs/scenarios/12-…md` |
| P4 동기화 정책 (백엔드 내부) | `docs/scenarios/09-startup-shutdown.md`, `p4_sync.py` |
