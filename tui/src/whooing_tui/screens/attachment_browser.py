...
동기화 자동 submit (CL #51124+, sync facade 경유):
- add 성공 시 → `[db, copied_file]` 한 단위로 submit. description 은
  `sync.describe_attachment_add(entry_id, filename, size, sha256)` 의
  기계적 한 줄 (LLM 미관여).
...
- 동기화 백엔드가 'none'(기본) 이면 모두 no-op — 비-P4 사용자는 완전 무시.
"""
...
def add_attachment(
    ...
    *,
    ...
    + submit 진행. App 종료 직전 `sync.flush_on_exit` 가 join.
    ...
    # 동기화 자동 submit — db + 디스크 파일을 한 단위로. 백엔드 없으면 no-op.
    from whooing_tui import sync
    sync.submit_files(
        [tui_data.db_path(), copied],
        sync.describe_attachment_add(
            entry_id=entry_id,
            filename=src.name,
            size_bytes=size,
            sha256=sha,
...
def update_note(attachment_id: int, note: str | None) -> dict[str, Any] | None:
    ...
    if info is None:
        return None
    # 동기화 자동 submit (db 만 — 디스크 파일 변경 X). 백엔드 없으면 no-op.
    from whooing_tui import sync
...
def remove(
    ...
    if info is None:
        return None
    # 동기화 자동 submit — 케이스에 따라 paths / description 분기. no-op if none.
    from whooing_tui import sync
    db = tui_data.db_path()
    ...
    sync.submit_files(
        paths,
        sync.describe_attachment_delete(
            entry_id=str(info.get("entry_id") or ""),
