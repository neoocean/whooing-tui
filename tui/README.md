...
| Esc | **noop** — 종료 안 함 | 컬럼 marker 해제 (파란은 유지) |
| `q` | 앱 종료 | 앱 종료 |
| `e` | 거래 수정 | 거래 수정 |
| `n` | 새 거래 입력 | 새 거래 입력 |
...
필터가 활성일 때는 status bar 가 노란색 (warn) 으로 `필터: left=x20 — 4/12건. c 로 해제 / r 로 재로드.` 와 같이 안내합니다. **종료는 q 만** (Esc 는 컬럼 marker 해제 전용 — 사용자 의도치 않은 종료 방지).
