...
중복**. 두 도구를 한 패키지로 묶지 않는 이유:

- mcp-server 는 `mcp>=1.0` / `playwright` / `pdfplumber` 등 무거운 의존성을
  싣는다. TUI 사용자는 이걸 받을 이유가 없다.
- mcp-server 의 `tools/`, `parsers/` 는 LLM 도구 정의에 묶여 TUI 와는
  결합이 다르다.
...
## 4. 후잉 API 사용 규칙 (mcp-server 와 공유)
...
6. **MCP 직접 호출 (선택)** — Phase 4 이후. 현재는 REST 만으로 충분.
   ✅ scaffolding 완료 (CL #50987). `mcp_bridge.py` 의 `WhooingMcpBridge`
   가 같은 monorepo 의 `whooing_mcp.official_mcp.OfficialMcpClient` 를
   wrap. UI 통합 (보고서 / 예산 / 자주입력 매칭 등의 화면) 은 후속 CL.
