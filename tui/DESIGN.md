...
flowchart TB
    ...
    PICKER -- "dismiss((sid, title))" --> ENTRIES
    PICKER -- "dismiss(None) — 취소" --> ENTRIES
    ENTRIES -- "a 키" --> ACCOUNTS["<b>AccountsScreen</b><br/>계정과목 조회 / CRUD"]
    ACCOUNTS -- "n / Enter / d" --> ACCDLG["AccountEditDialog<br/>+ ConfirmModal"]
    ACCDLG --> ACCOUNTS
    ACCOUNTS -- "q / Esc → dismiss" --> ENTRIES
    ENTRIES -- "n / Enter / d" --> ENTDLG["EntryEditDialog<br/>+ ConfirmModal"]
    ENTDLG --> ENTRIES
    ENTRIES -- "q / Esc" --> EXIT[app.exit]
```

**이력**: v0.7.x 까지는 HomeScreen (섹션 picker + 계정과목 트리) 이 초기
화면이고 EntriesScreen 은 `e` 키로 push 됐다. CL #51023 에서 사용자
지시로 초기 화면을 EntriesScreen 으로 바꾸고 옵션을 분리. HomeScreen
제거.
