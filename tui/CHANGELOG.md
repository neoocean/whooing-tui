...
- **계정 항목 정렬 Tree UI** (감사 잔여 / 로드맵 P3-C 완성) — `계정과목` 화면
...
- `EntryEditDialog` 버튼 행이 **좁은 폭에서 가로로 넘쳐** 'Cancel' 이 잘리던
  문제 수정(사용자 보고: 스크롤을 끝까지 내려도 버튼이 다 안 보임). 버튼
  `min-width:18` 고정 → `width:1fr; max-width:24` 로 가용 폭을 반씩 나눠 갖게,
  `button-row height:auto`, 프레임 좌우 padding `1 2`→`1 1`. 통합 테스트에
  버튼 가로 포함 검증 추가(40×14·50×18·98×30).
- 스크린샷 생성기 결정성: Textual 의 랜덤 `terminal-<id>` → 장면별 고정 id,
  헤더 시계(HH:MM:SS) 고정값 정규화 → `make gen-images` 재실행 시 실제 시각
  변경만 diff. 현재 인터페이스로 6장 전부 재생성, MANUAL 에 작은 화면 스크롤
  안내 추가.
